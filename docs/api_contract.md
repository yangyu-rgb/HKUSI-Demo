...
- `GET /api/demo/v2-model`：返回 AI v2.1 候选排行榜、最终参数、时间切分、数据审计、误差切片、区间覆盖、漂移参考、晋级检查和运行时产物状态。
...
用户可见等待时间默认由 AI v2.1 预测。模型根据官方历史客流压力、口岸、方向、小时、星期、节假日、天气和事件强度计算分钟基线；新鲜官方等级在三小时内按新鲜度和跨度校准。模型异常时自动降级到时间加权统计模型并保留可用官方校准。

AI v2.1 按“模型基线—官方等级—众包—不确定性”顺序计算。众包沿用质量、新鲜度和跨度衰减，最大权重15%；模型降级后也从未校准统计基线开始，避免重复修正。全部运行时调整量进入附加不确定性。
...
- AI v2.1 使用真实公开客流特征与生成等待标签驱动课堂 Demo；AI v1 仅作影子比较。真实数据 readiness 只作为生产研究边界。
