...
Demo 模型先对口岸预测点进行线性插值，再融合相似工作日或周末、小时和天气历史样本；存在近期众包数据时也会加入计算。参与计算的众包等待时间按质量分加权，预测依据会返回平均质量分和实际样本数。历史标准差和局部预测斜率决定区间宽度，正态分布用于把剩余通关时间换算为迟到概率。以后可以在不改变 API 边界的情况下替换为生产模型。
