...
- `GET /api/demo/v2-readiness`：基于已关联的高质量实际等待标签，返回标签数、口岸/日期/小时切片覆盖、时间切分、统计与影子误差、输入源状态和 V2 实验/生产晋级判断。
...
关联的反馈只会在未过期、质量为 `high` 且该预测口岸尚无实际标签时写入训练标签；低质量或重复标签仍保留关联，但不会污染 V2 训练快照。
