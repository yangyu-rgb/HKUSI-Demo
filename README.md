...
离线 AI v1 只在合成数据上训练，用于验证数据、训练、评估和模型产物流程。其指标不代表真实口岸效果，当前 FastAPI 仍使用可解释统计模型。
...
本清单按优先级而非日期维护。当前下一项功能是**AI v1 影子模式接入与降级验证**。
...
- [ ] 设计 AI v1 模型加载器、特征适配和版本校验
- [ ] 以影子模式同时计算统计模型和 AI v1，不改变默认用户结果
- [ ] 记录两模型差异并验证模型缺失或不兼容时自动降级
- [ ] 计算订阅下一次有效通勤日期和到达时间
