...
test("3D 口岸模块在桌面断点内不超过视口且视觉层不越界", async ({ page }) => {
  for (const viewport of [
    ...
    const scene = page.getByRole("region", { name: /Hong Kong and Shenzhen four-port geographic flow/ });
...
test("移动端路线控制保持单行横向滚动且页面没有横向溢出", async ({ page }) => {
  for (const viewport of [
    ...
    const panel = page.locator('section[aria-labelledby="flow-title"]');
    const routes = page.getByLabel("Four-port route focus controls");
...
test("拖动场景不会滚动页面或破坏路线聚焦功能", async ({ page }) => {
  ...
  const scene = page.getByRole("region", { name: /Hong Kong and Shenzhen four-port geographic flow/ });
  const canvas = scene.locator("canvas");
  const autoTour = page.getByRole("button", { name: "Auto tour On" });
  await expect(canvas).toBeVisible();
  ...
  const returnOverview = page.getByRole("button", { name: "Overview" });
  ...
  const firstRoute = page.getByLabel("Four-port route focus controls").getByRole("button").first();
...
test("路线详情跟随锚点并夹紧在场景内，键盘操作会暂停巡航", async ({ page }) => {
  ...
  const scene = page.getByRole("region", { name: /Hong Kong and Shenzhen four-port geographic flow/ });
  const visual = scene.locator("..");
  const autoTourOn = page.getByRole("button", { name: "Auto tour On" });
  ...
  await page.getByRole("button", { name: "Auto tour Off" }).click();
  await scene.focus();
  await page.keyboard.press("ArrowRight");
  await expect(page.getByRole("button", { name: "Auto tour Paused" })).toHaveAttribute("aria-pressed", "true");
});
...
test("减少动态效果时关闭连续巡航但保留显式路线聚焦", async ({ page }) => {
  ...
  const autoTour = page.getByRole("button", { name: "Auto tour Off" });
...
test("3D 场景离开视口时停止渲染并在返回后保留路线选择", async ({ page }) => {
  ...
  const autoTour = page.getByRole("button", { name: /Auto tour/ });
...
test("WebGL context 丢失时显示文字回退并在恢复后重启场景", async ({ page }) => {
  ...
  const scene = page.getByRole("region", { name: /Hong Kong and Shenzhen four-port geographic flow/ });
  const canvas = scene.locator("canvas");
  const fallback = page.getByText("This browser cannot run the 3D scene");
  await scene.scrollIntoViewIfNeeded();
  await expect(canvas).toBeVisible();
  await expect(scene).toHaveAttribute("data-render-state", "running");
