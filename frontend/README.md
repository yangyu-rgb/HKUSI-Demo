...
- `/model`：V1 合成数据指标、分口岸误差、运行时影子差异、V1 Demo 就绪度和 V2 真实标签门槛。
...
`npm run check:api` 会重新生成 OpenAPI 文件；如果已提交的契约文件不是最新版本，命令会失败。Playwright 会用 Chromium 覆盖桌面与移动端双向规划、通知、模型门槛，并对六个路由执行 axe WCAG A/AA 检查。
