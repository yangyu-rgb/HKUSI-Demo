...
- `/crowdsource`：课堂反馈、方向/通道、重复提交拦截、质量评分、最多30%透明校准和有效期动态列表。
...
`npm run check:api` 会重新生成 OpenAPI 文件；如果已提交的契约文件不是最新版本，命令会失败。Playwright 会分别启动并等待后端存活探针与前端页面，再用 Chromium 覆盖双向规划、场景实验室、通知、模型说明和独立手机首页，并对八个路由执行 axe WCAG A/AA 检查。CI 使用单 worker，避免并行重置共享 Demo SQLite 状态。
