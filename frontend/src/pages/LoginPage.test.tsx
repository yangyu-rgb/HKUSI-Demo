...
describe("LoginPage", () => {
  it("selects a local workspace and records the demo session", async () => {
    ...
    render(<QueryClientProvider client={client}><MemoryRouter initialEntries={["/login"]}><Routes><Route path="login" element={<LoginPage />} /><Route path="/" element={<h1>已进入</h1>} /></Routes></MemoryRouter></QueryClientProvider>);
    fireEvent.click(await screen.findByRole("button", { name: /跨境通勤者/ }));
    fireEvent.click(screen.getByRole("button", { name: /进入 CrossBorder AI/ }));
    expect(await screen.findByRole("heading", { name: "已进入" })).toBeInTheDocument();
